...
def init_socketio(sio):
    ...
    def handle_join(data):
        ...
        if user_id:
            join_room(user_id)
            print(f"[JOIN] User {user_id} joined room {user_id}")
...
def send_message():
    ...
    if socketio:
        ...
        if sender_id != target_user:
            socketio.emit('new_message', socket_message, room=target_user)
...
# ==================== CHUNKED UPLOAD ENDPOINTS FOR 3GB FILES ====================

@message_bp.route('/api/upload-chunk', methods=['POST'])
def upload_chunk():
    ...
    try:
        ...
        if not chunk or file_id is None or filename is None:
        ...
        print(f"📦 [UPLOAD] Chunk {chunk_number + 1}/{total_chunks} saved for {filename}")
        ...
        print(f"❌ [UPLOAD] Chunk upload error: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500

@message_bp.route('/api/finalize-upload', methods=['POST'])
def finalize_upload():
    ...
    try:
        ...
        # Paths
        temp_dir = os.path.join(UPLOAD_FOLDER, 'temp', file_id)
        final_dir = os.path.join(UPLOAD_FOLDER, 'files')
        ...
        relative_path = os.path.join('files', safe_filename).replace('\\', '/')
        
        print(f"✅ [UPLOAD] File completed: {filename} ({actual_size} bytes)")
        
        return jsonify({
            'success': True,
            'filePath': relative_path,
            'serverPath': final_path,
        ...
        print(f"❌ [UPLOAD] Finalize error: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500